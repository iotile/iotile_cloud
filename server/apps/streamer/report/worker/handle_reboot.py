...
from apps.utils.aws.redshift import get_time_difference_server_utc, get_ts_from_redshift
...
BASE_RETRYDELAY_SECONDS = 3 * 60  # in seconds
        ...
        if 'device_slug' in arguments and 'block_start_id' in arguments and 'reboot_ids' in arguments and 'block_end_id' in arguments and 'project_id' in arguments:
            ...
            if checker.ready_to_process():
                ...
                if checker.continue_delay():
                    delay = BASE_RETRYDELAY_SECONDS * checker.attempts
                    logger.info('Data not ready to process. Schedule to retry after {} seconds'.format(delay))
                    HandleRebootAction.schedule(args=arguments, delay_seconds=delay)
        else:
