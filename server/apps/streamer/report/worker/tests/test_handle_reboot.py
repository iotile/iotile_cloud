...
class HandleRebootTestCase(TestMixin, TestCase):
    ...
    def testHandleReboot(self, mock_download_s3, mock_handle_reboot):
        queue = QueueTestMock()
        ...
        self.assertEqual(str(streamer_data_report_1.timestamp.isoformat()), sent_timestamp_1.isoformat())
        ...
        self.assertEqual(str(streamer_data_report_2.timestamp.isoformat()), sent_timestamp_1.isoformat())
        ...
        self.assertEqual(str(streamer_data_report_3.timestamp.isoformat()), sent_timestamp_2.isoformat())
        ...
        self.assertEqual(str(streamer_data_report_4.timestamp.isoformat()), sent_timestamp_2.isoformat())
        self.assertEqual(streamer_data_report_4.status, 'cln')

        # No longer trying to set status
        self.assertEqual(StreamData.objects.filter(device_slug=self.pd1.slug, status='unk').count(), 24)
        self.assertEqual(StreamData.objects.filter(device_slug=self.pd1.slug, status='cln').count(), 37)
    ...
    def testCheckStreamerDelayRedshift(self, mock_download_s3, mock_handle_reboot, mock_schedule):
        ...
        StreamData.objects.create(stream_slug=formatted_gsid(pid=self.p1.formatted_gid, did=self.pd1.formatted_gid, vid='5c00'),
                                  ...
                                  timestamp=base_dt,
                                  int_value=0)
        process_report_action = ProcessReportAction()
        ...
        mock_schedule.assert_called_with(args=reboot_args, delay_seconds=180)
