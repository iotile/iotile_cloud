...
class StreamDataCountHelper(object):
    ...
    def __init__(self, device):
        ...
        self.data_mask = get_data_mask_date_range(device)

        project_slug = device.project.slug if device.project else 'p--0000-0000'
        distinct_data_stream_qs = DataManager.filter_qs('data', device_slug=device.slug, project_slug=project_slug)
        distinct_event_streams_qs = DataManager.filter_qs('event', device_slug=device.slug, project_slug=project_slug)
        ...
        for streamid in StreamId.objects.filter(device=device, block__isnull=True):
